...
# =============================================================================
# D3 - Density-dependent fecundity
# =============================================================================
D3_RADIUS_M = 3
D3_B = 0.01  # PROVISIONAL DD coefficient; must be calibrated.


def calculate_density_dependent_fecundity(
        adults: pl.DataFrame,
        population: pl.DataFrame | None = None,
        f_max: float = 1.0,
        b: float = D3_B,
        radius_m: float = D3_RADIUS_M,
) -> pl.DataFrame:
    """
    Add density-dependent expected fecundity and an integer stochastic egg count to adult rows.

    Supplied equation:
        F_t = F_max * exp(-b * N_t)

    N_t is the number of other individuals in the DD range. ``population`` can therefore include
    larvae/eggs/pupae as well as adults. If omitted, only adults contribute to N_t.

    Integer ``eggs_laid`` uses stochastic rounding, so its expectation equals ``expected_fecundity``.
    """
    if f_max < 0:
        raise ValueError("f_max must be >= 0.")
    if b < 0:
        raise ValueError("b must be >= 0 for compensatory density dependence.")
    if population is None:
        population = adults

    enriched = _attach_local_other_count(
        adults,
        population,
        radius_m,
        ...
        expected_fecundity=f_max * (-b * pl.col("local_other_individuals")).exp()
    )

    return (
        enriched
        .with_columns(
            _fecundity_floor=pl.col("expected_fecundity").floor(),
            _fecundity_draw=polars_random.uniform(),
        )
        .with_columns(
            eggs_laid=(
                pl.col("_fecundity_floor")
                + (
                    pl.col("_fecundity_draw")
                    < (pl.col("expected_fecundity") - pl.col("_fecundity_floor"))
                ).cast(pl.Int64)
            ).cast(pl.Int64)
        )
        .drop("_fecundity_floor", "_fecundity_draw")
    )


# Integration point during reproduction:
# reproductive_adults = calculate_density_dependent_fecundity(
#     adults=reproductive_adults,
#     population=all_individuals,
#     f_max=temperature_dependent_fecundity,
# )
