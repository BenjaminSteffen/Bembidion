...
# =============================================================================
# D6 - Update the density map every simulation day
# =============================================================================
...
def update_density_map(
    ...
    """
    Rebuild per-cell adult and larval density from the CURRENT population.

    The output has the same landscape cells plus ``adult_density`` and ``larval_density``. When the
    original movement-only dataframe has no ``lifestage`` column, all individuals are treated as adults.
    Empty cells are explicitly filled with zero.
    """
    if "lifestage" in individuals.columns:
        adult_counts = (
            individuals.filter(pl.col("lifestage") == "Adult")
            .group_by("x", "y").agg(pl.len().alias("adult_density"))
        )
        larval_counts = (
            individuals.filter(pl.col("lifestage").is_in(("Larva", "Larvae", "L1", "L2", "L3")))
            .group_by("x", "y").agg(pl.len().alias("larval_density"))
        )
    else:
        adult_counts = individuals.group_by("x", "y").agg(pl.len().alias("adult_density"))
        larval_counts = pl.DataFrame(
            schema={"x": landscape.schema["x"], "y": landscape.schema["y"], "larval_density": pl.UInt32}
        )

    return (
        landscape
        .join(adult_counts, on=("x", "y"), how="left")
        .join(larval_counts, on=("x", "y"), how="left")
        .with_columns(
            adult_density=pl.col("adult_density").fill_null(0),
            larval_density=pl.col("larval_density").fill_null(0),
        )
    )


def daily_movement_and_density_update(
        individuals: pl.DataFrame,
        landscape: pl.DataFrame,
        max_distance: int = 14,
        turning_probability: float = 0.8,
) -> tuple[pl.DataFrame, pl.DataFrame]:
    """
    Run unchanged movement and refresh density for the original adult-only movement dataframe.

    A Population.py-style dataframe with multiple lifestages should use its stage-specific movement
    scheduler first and then call update_density_map() directly, because the original move_to() only
    returns x/y/direction and intentionally does not preserve arbitrary lifecycle columns.
    """
    moved = move_to(individuals, landscape, max_distance, turning_probability)
    density_map = update_density_map(moved, landscape)
    return moved, density_map


# Example annual scheduling:
# density_map = update_density_map(bembidions, movement_map)  # day 0
# for day in range(365):
#     bembidions, density_map = daily_movement_and_density_update(bembidions, movement_map)
#     # Any density-dependent process executed after movement reads THIS day's density_map.
