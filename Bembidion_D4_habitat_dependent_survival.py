...
# =============================================================================
# D4 - Habitat-dependent survival via habitat-scaled carrying capacity
# =============================================================================
D4_RADIUS_M = 3
D4_REFERENCE_CAPACITY = 20.0  # PROVISIONAL individuals in the DD neighbourhood at quality = 1.


def apply_habitat_dependent_survival(
        individuals: pl.DataFrame,
        landscape: pl.DataFrame,
        habitat_quality_column: str = "habitat_quality",
        radius_m: float = D4_RADIUS_M,
        reference_capacity: float = D4_REFERENCE_CAPACITY,
        baseline_survival: float = 1.0,
) -> pl.DataFrame:
    """
    Exploratory D4 implementation: habitat quality scales local carrying capacity.

        K_local = reference_capacity * habitat_quality
        S_habitat = baseline_survival * min(1, K_local / N_local)

    ``habitat_quality`` must be in [0, 1]. N_local includes the focal individual. This is intentionally
    a simple carrying-capacity interpretation because D4 is explicitly described as requiring further
    investigation in the supplied specification.
    """
    if habitat_quality_column not in landscape.columns:
        raise ValueError(
            f"landscape must contain a '{habitat_quality_column}' column with values in [0, 1]"
        )
    if reference_capacity <= 0:
        raise ValueError("reference_capacity must be > 0.")
    if not (0 <= baseline_survival <= 1):
        raise ValueError("baseline_survival must be in [0, 1].")

    local = _attach_local_other_count(
        individuals,
        individuals,
        radius_m,
        output_column="local_other_individuals",
        subtract_self=True,
    ).with_columns(local_individuals=pl.col("local_other_individuals") + 1)

    local = local.join(
        landscape.select("x", "y", pl.col(habitat_quality_column).alias("habitat_quality")),
        on=("x", "y"),
        how="left",
    )

    if local.select(pl.col("habitat_quality").is_null().any()).item():
        raise ValueError("Some individuals occupy cells without habitat_quality information.")

    local = (
        local
        .with_columns(
            habitat_quality=pl.when(pl.col("habitat_quality") < 0).then(0.0)
            .when(pl.col("habitat_quality") > 1).then(1.0)
            .otherwise(pl.col("habitat_quality")),
        )
        .with_columns(local_capacity=reference_capacity * pl.col("habitat_quality"))
        .with_columns(
            habitat_survival_probability=(
                baseline_survival
                * pl.when(pl.col("local_individuals") <= pl.col("local_capacity"))
                .then(1.0)
                .otherwise(pl.col("local_capacity") / pl.col("local_individuals"))
            ),
            _habitat_draw=polars_random.uniform(),
        )
        .filter(pl.col("_habitat_draw") < pl.col("habitat_survival_probability"))
        .drop(
            "local_other_individuals", "local_individuals", "habitat_quality",
            "local_capacity", "habitat_survival_probability", "_habitat_draw"
        )
    )
    return local


# Required landscape addition before using D4:
# movement_map = movement_map.with_columns(habitat_quality=... )  # calibrated values in [0, 1]
# Recommended once-daily order:
# bembidions = apply_habitat_dependent_survival(bembidions, movement_map)
