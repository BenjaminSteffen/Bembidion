...
# =============================================================================
# D2 - Density-dependent larval survival; adult DD disabled
# =============================================================================
D2_RADIUS_M = 3
D2_S0 = 1.0          # Density-only survival multiplier; background survival can be applied separately.
D2_A = 0.01          # PROVISIONAL DD coefficient; must be calibrated.


def apply_larval_density_dependent_survival(
        individuals: pl.DataFrame,
        density_population: pl.DataFrame | None = None,
        radius_m: float = D2_RADIUS_M,
        s0: float = D2_S0,
        a: float = D2_A,
) -> pl.DataFrame:
    """
    Apply D2 survival to larvae only, following the supplied equation

        S_t = S_0 / (1 + a * N_t)

    where N_t is the number of other individuals within the DD radius, matching the wording of the
    supplied specification. Adults contribute to N_t by default but receive no density-dependent
    mortality themselves. Pass a larval-only ``density_population`` if the intended mechanism is
    specifically larva-larva competition. ``s0=1`` makes this a density-only survival modifier.
    """
    if not (0 <= s0 <= 1):
        raise ValueError("s0 must be in [0, 1].")
    if a < 0:
        raise ValueError("a must be >= 0 for compensatory density dependence.")
    if "lifestage" not in individuals.columns:
        # The original movement-only dataframe contains adults only, so D2 leaves it unchanged.
        return individuals

    pop = individuals.with_row_index("_dd_order")
    larvae = _larval_subset(pop)
    non_larvae = pop.filter(~pl.col("lifestage").is_in(("Larva", "Larvae", "L1", "L2", "L3")))

    if larvae.is_empty():
        return individuals

    if density_population is None:
        neighbours = pop
    else:
        # Keep only the spatial columns required for counting. The focal larvae are assumed to be
        # represented in density_population when subtract_self=True.
        neighbours = density_population

    larvae = (
        _attach_local_other_count(
            larvae, neighbours, radius_m, output_column="local_other_individuals", subtract_self=True
        )
        .with_columns(
            dd_survival_probability=s0 / (1.0 + a * pl.col("local_other_individuals")),
            _dd_draw=polars_random.uniform(),
        )
        .filter(pl.col("_dd_draw") < pl.col("dd_survival_probability"))
        .drop("local_other_individuals", "dd_survival_probability", "_dd_draw")
    )

    return (
        pl.concat([larvae, non_larvae], how="vertical_relaxed")
        .sort("_dd_order")
        .drop("_dd_order")
    )


# Integration point in a Population.py-style lifecycle:
# individuals = apply_larval_density_dependent_survival(individuals)
# No adult density-dependent mortality is called in D2.
